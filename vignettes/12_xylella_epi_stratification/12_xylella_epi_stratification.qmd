---
title: "Vector-Borne Disease Stratification"
subtitle: "Categorical Construction of Eco-Epidemiological Models"
author: "Simon Frost"
format:
  html:
    code-fold: false
...
For a given tree infection prevalence $\pi$ (fraction of trees that are infectious), the 2×2 infection transition matrix for adult vectors is:

$$
\mathbf{T}_{\text{inf}} = \begin{pmatrix} 1 - \beta_{VP} \pi & 0 \\ \beta_{VP} \pi & 1 \end{pmatrix}
$$
...
The stratified matrix has dimension $n_{\text{stages}} \times n_{\text{infection}} = 3 \times 2 = 6$, with block structure:

$$
\mathbf{A}_{\text{strat}}[(s_{\text{to}}, i), (s_{\text{from}}, j)] = \mathbf{T}_{\text{inf}}[s_{\text{to}}, s_{\text{from}}] \cdot \mathbf{A}_{\text{demo}}[i, j]
$$

where $s$ indexes infection status (S, I) and $i,j$ index lifecycle stages (egg, nymph, adult).
...
The tree transition matrix has $\lambda \approx 1$ because trees neither reproduce nor die rapidly at these parameter values — the dominant eigenvalue deviates from 1 only through disease-induced mortality.

## Basic Reproduction Number

The basic reproduction number $R_0$ quantifies whether the pathogen can invade a fully susceptible population. We compute it using the **next-generation matrix** (NGM) approach. For the vector-tree system, the infected compartments are: infective vectors and infected trees (latent + symptomatic).

The NGM decomposes the infection dynamics into a **transmission matrix** $\mathbf{F}$ (new infections) and a **transition matrix** $\mathbf{V}$ (changes in state among infected compartments). Then $R_0 = \rho(\mathbf{F}\mathbf{V}^{-1})$.
...
A key advantage of the product structure is that we can efficiently sweep epidemiological parameters while keeping the demographic model fixed. Here we vary tree prevalence $\pi$ from 0 to 1 and examine how the dominant eigenvalue of the stratified vector model changes:
...
Let us compare the eigenstructure at the disease-free equilibrium ($\pi = 0$) and at an endemic level ($\pi = 0.5$):
...
We can also sweep the transmission rate $\beta_{VP}$ to find how it influences $R_0$:
...
- **Next-generation matrix**: $R_0$ follows naturally from the product decomposition into transmission and transition components
