---
title: "Stratification and Coarsening"
author: "Simon Frost"
format:
  html:
    code-fold: false
...
- **Coarsening** — aggregates a fine-resolution model to a coarser one via pushforward along a `FinFunction`
...
With symmetric dispersal, $\lambda$ is preserved — the metapopulation grows at the same rate as an isolated patch.
...
Progressive and direct coarsening agree — the pushforward is functorial (composition of FinFunctions commutes).
...
The small difference comes from the coarsening approximation, not from any failure of commutativity.
