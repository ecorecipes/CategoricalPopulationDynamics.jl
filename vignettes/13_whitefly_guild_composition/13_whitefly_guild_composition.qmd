---
title: "Enemy Guild Composition"
subtitle: "Categorical Assembly of Multi-Parasitoid Biocontrol Models"
author: "Simon Frost"
format:
  html:
    code-fold: false
...
$$
\mathbf{A}_{\text{full}} = \mathbf{A}_{\text{whitefly}} + \Delta\mathbf{A}_{P_1} + \Delta\mathbf{A}_{P_2} + \Delta\mathbf{A}_{P_3}
$$

where each $\Delta\mathbf{A}_{P_i}$ is a **sparse, negative** perturbation capturing the mortality imposed by parasitoid $i$ on specific whitefly stages. This is exactly the additive kernel composition that `compose_transitions` and `oapply` implement.
...
| Species | Model organism | Strategy | Attacks | Rate (α) |
|---------|---------------|----------|---------|-----------|
| P₁ | *Eretmocerus* sp. | Primary parasitoid | nymph1, nymph23 | 0.03/day |
| P₂ | *Encarsia* (obligate) | Obligate autoparasitoid | nymph23 only | 0.025/day |
| P₃ | *Encarsia* (facultative) | Facultative autoparasitoid | nymph1, nymph23 | 0.02/day |

Each parasitoid imposes **additional daily mortality** on the stages it attacks, reducing the stasis (within-stage survival) entries. We model each as a sparse matrix $\Delta\mathbf{A}_{P_i}$ with negative entries only at the attacked stages:
...
How sensitive is whitefly control to parasitism intensity? We sweep the parasitism rates of P₁ and P₂ (keeping P₃ at its baseline) and plot the resulting $\lambda$ surface:
...
The black contour line shows the **critical parasitism boundary** where $\lambda = 1$. Combinations above and to the right of this line achieve pest suppression.
...
5. **Intensity sweeps** — the $\lambda$ surface shows critical parasitism boundaries for pest control
