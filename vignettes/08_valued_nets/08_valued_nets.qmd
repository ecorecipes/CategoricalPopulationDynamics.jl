...
2. **`transition_matrix(vnet, name)`** — materializes a single transition to a dense matrix
3. **`to_matrix(vnet)`** — sums all transitions into the full projection matrix $\mathbf{A}$
4. **`lower(vnet, MPMTarget())`** — converts to a `MatrixProjectionModel` for downstream analysis
5. **Named transitions** — enable process-level decomposition (stasis vs progression vs reproduction) while maintaining full compatibility with matrix-based analyses
